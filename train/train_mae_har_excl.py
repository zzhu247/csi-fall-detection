...
# NOTE: META_PATH/SPLITS_DIR are now built inside main() from --task, not fixed constants,
# so this file can pretrain on any Multitask/<TaskName> sub-dataset (e.g. HumanIdentification)
# without touching every downstream reference. Kept as module-level names here (still pointing
# at HumanActivityRecognition) ONLY for backward compatibility with any external script that
# imports META_PATH/SPLITS_DIR directly from this module (e.g. run_l2sp_sweep.py,
# run_norm_comparison.py, add_mlp_probe_to_results.py all do `from train_mae_har import ...`
# but none of them currently import META_PATH/SPLITS_DIR, so this is a safety net, not a known
# breakage -- but don't remove these two lines without checking those scripts first).
...
def load_split(name, meta, label_map, splits_dir=None):
    """splits_dir defaults to the module-level SPLITS_DIR (HumanActivityRecognition) if not
    given, preserving old call sites' behavior exactly. main() passes the --task-specific
    splits_dir explicitly so this works correctly for HumanIdentification too."""
...
def main():
    ...
    parser.add_argument('--exclude_devices', default='',
                         help="Comma-separated device names to drop from ALL data (train_id AND "
                              "every OOD split) before anything else -- e.g. --exclude_devices "
                              "AmazonPlug. Applied once, at the metadata level, so train_id and "
                              "every split derived from it are automatically, consistently "
                              "filtered -- no separate logic needed per split. Motivation: "
                              "AmazonPlug has only 56 real subcarriers vs. the 232 every other "
                              "device provides (76%% zero-padding after pad_csi) -- a structural "
                              "outlier discovered during the domain-adversarial investigation, "
                              "candidate confound for Cross-Device OOD numbers specifically. "
                              "Empty (default) = no exclusion, byte-identical to prior behavior.")
    ...
    exclude_devices = [d.strip() for d in args.exclude_devices.split(',') if d.strip()]
    excl_tag = f"_excl{'-'.join(exclude_devices)}" if exclude_devices else ""
    ...
    exp_name = (f"mae_har_ep{args.epochs}_mask{args.mask_ratio}_strategy{args.mask_strategy}_ph{args.patch_h}pw{args.patch_w}_seed{args.seed}"
                f"_enc{args.encoder_depth}_dim{args.encoder_dim}_bs{args.batch_size}{excl_tag}")
    print(f"\nExperiment: {exp_name}")
    ...
    if exclude_devices:
        before_n = len(meta)
        for dev in exclude_devices:
            dev_n = (meta['device'] == dev).sum()
            print(f"[exclude-devices] dropping device={dev!r}: {dev_n} rows")
        meta = meta[~meta['device'].isin(exclude_devices)].reset_index(drop=True)
        print(f"[exclude-devices] meta: {before_n} -> {len(meta)} rows "
              f"(applies to train_id AND every OOD split, since both are derived from this "
              f"filtered meta going forward)")
    import json as _json
